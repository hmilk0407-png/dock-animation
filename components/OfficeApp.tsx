...
export default function OfficeApp({
  ...
  /** 受付ミルクのお知らせ (朝の予定読み上げ) → ChatPanel へ */
  ...
  /* ---------- 朝の予定読み上げ: その日オフィスに入った最初の1回だけ ----------
     /api/calendar/today は Apps Script が届けた当日分を返す。
     まだ届いていない (received=false) 場合は何も言わず、次回開いた時に再試行する。 */
  useEffect(() => {
    if (uiMode !== "office") return;
    const key = `buhi:morning:${todayJst()}`;
    try {
      if (window.localStorage.getItem(key)) return;
    } catch {
      /* localStorage 不可の環境では毎回読み上げる */
    }
    let cancelled = false;
    (async () => {
      try {
        ...
        if (cancelled || !data?.received || !data?.text) return;
        try {
          window.localStorage.setItem(key, "1");
        } catch {
          /* noop */
        }
        setAnnouncement({ text: data.text, ts: Date.now() });
      } catch {
        /* 取得失敗は静かに諦める (通常のチャットは使える) */
      }
    })();
    return () => {
      cancelled = true;
